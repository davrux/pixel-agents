#!/usr/bin/env node
/**
 * Does the security check actually catch anything?
 *
 ...
 * security.mjs gets a hole punched into the real source here, and must fail on
 * it. Every case below is a hole somebody could plausibly leave behind — an
 * ungated route, a handler acting on a payload's userId, a voice token minted
 * for whoever asked.
 *
...
const files = [...new Set(CASES.map(([, f]) => f))];
...
for (const [name, rel, anchor, replacement, expect] of CASES) {
  ...
  let code = 0;
  try {
    out = execFileSync('node', [join(HERE, 'security.mjs')], { cwd: ROOT, encoding: 'utf8' });
  } catch (e) {
    out = `${e.stdout ?? ''}`;
    code = e.status ?? 1;
  } finally {
  ...
  if (code !== 0 && out.toLowerCase().includes(expect.toLowerCase())) ok++;
  else missed.push(`${name} — went unnoticed (exit ${code})`);
}

if (missed.length === 0) {
  console.log(`  \x1b[32m✓ PASS\x1b[0m  security self-test: all ${CASES.length} planted holes were caught`);
  process.exit(0);
}
console.log(`  \x1b[31m✗ FAIL\x1b[0m  security self-test: ${missed.length} of ${CASES.length} holes slipped through:`);
missed.forEach((m) => console.log(`        - ${m}`));
console.log('        A rule that cannot fail is not a check — fix security.mjs (or this case, if the code moved).');
process.exit(1);
