...
export interface Principal {
  ...
  isAdmin: boolean;
  /** Account role. `user` may create + edit their own rooms; `customer` may not
   *  create or edit anything. Absent → treated as `user` (backwards compatible). */
...
export function can(
  ...
  // A `user` may create their own rooms (they become that zone's admin on
  // creation); a customer may not create anything.
  if (capability === 'zone.create') return !!principal.userId && !isCustomer;
