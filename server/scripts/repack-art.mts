...
for (const type of PACKED_ART_TYPES) {
  const rows = db.prepare('SELECT name, data FROM assets WHERE type = ?').all(type) as Array<{ name: string; data: string }>;
  for (const r of rows) {
    let parsed: unknown;
    try {
    ...
    const after = JSON.stringify(repacked);
    ...
    packed++;
    console.log(`  ${type}/${r.name}: ${kb(r.data.length)} → ${kb(after.length)}`);
