...
function syncTilesets(): string[] {
  ...
  for (const file of fs.readdirSync(TILED_DIR).sort()) {
    ...
    if (!dirty) continue;
    touched.push(file);
    // A tileset is OUR artifact — generated by the bake scripts and written in this
    // formatting — so re-serializing it is the normal thing here. A MAP belongs to
    // Tiled and is edited in place instead (see syncZoneMaps).
...
function syncZones(): string[] {
  ...
  for (const file of fs.readdirSync(zonesDir).sort()) {
    ...
    // Decisions come from the parsed model, writes go through jsonEdit: a map is
    // written by TILED, and re-serializing it here turned a one-field change into a
    // 25 000-line diff — after which the next save in Tiled produced the reverse.
    ...
    for (const layer of json.layers ?? []) {
      for (const obj of layer.objects ?? []) {
        ...
        if (!obj.type && obj.gid && classOf.get(obj.gid & GID_FLAGS) === 'FurnitureTile') {
          ...
          if (!CHECK_ONLY) raw = setStringField(raw, `"id":${obj.id}`, 'type', 'FurnitureObject');
        ...
        for (const p of obj.properties) {
          if (!allowed.has(p.name)) {
            changes.push(`${file} object ${obj.id}: − ${p.name}`);
            // Anchored on the property's own object, which is unique per name within
            // this placement — the whole map may hold many `"name":"label"`, so the
            // anchor is searched inside this object's text, not the file's.
            if (!CHECK_ONLY) raw = removeObjectWithin(raw, `"id":${obj.id}`, `"name":${JSON.stringify(p.name)}`);
          }
        }
        // An emptied properties array is valid JSON but not a shape Tiled writes, so
        // the key goes with the last property rather than lingering as `[]`.
        if (!CHECK_ONLY && kept.length === 0) raw = removeField(raw, `"id":${obj.id}`, 'properties');
        obj.properties = kept;
        dirty = true;
      }
    }
    ...
    touched.push(file);
    if (!CHECK_ONLY) {
      // Sanity before writing: the edited TEXT must parse, and hold the same objects
      // we decided on. A text edit that silently broke the file would otherwise only
      // surface at the next import.
      const check = JSON.parse(raw) as { layers?: Array<{ objects?: unknown[] }> };
